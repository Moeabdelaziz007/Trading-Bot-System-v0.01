...
export default function ConnectWallet() {
    ...
    const handleConnect = async (providerId: string) => {
        ...
        try {
            // For Coinbase, initiate OAuth flow
            if (providerId === 'coinbase') {
                const response = await fetch('/api/proxy/auth/coinbase/initiate');
                ...
                // For other providers, simulate connection for now
    ...
    const handleDisconnect = async (providerId: string) => {
        try {
            // For Coinbase, call disconnect endpoint
            if (providerId === 'coinbase') {
                const response = await fetch('/api/proxy/auth/coinbase/disconnect', {
                    method: 'POST'
                });
                const data = await response.json();
                ...
                // For other providers, simulate disconnection
                setProviders((prev) =>
                    prev.map((p) =>
                        p.id === providerId ? { ...p, connected: false } : p
                    )
                                        ...
                                        {provider.connected ? (
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button
                                                    ...
                                                    aria-label={`قطع الاتصال بـ ${provider.name}`}
                                                >
                                                    قطع
                                                </button>
                                            </div>
